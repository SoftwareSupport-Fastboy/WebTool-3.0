...
// Hàm khởi tạo chuỗi lặp lại cho thông báo
...
// Hàm bấm vào thông báo sẽ hiển thị nội dung; Chỉ cần click vào thông báo sẽ hiển thị nội dung của nó trong container thông báo
...
// Đóng Container_With_Close_Button bằng nút "Close_Function_Button"
...
function handleClickPaste(input) {
    navigator.clipboard.readText()
        ...
        .catch(() => {
            StatusUpdateBox("Web-Tools 3.0", "Không thể đọc clipboard. Trình duyệt không cho phép hoặc chưa copy gì.", "red");
        });
}



// Cách xử lý việc Click Paste với ID lấy số
async function handleClickPasteWithIDTakeNumbers(id) {
    try {
        ...
        if (inputElement) {
            ...
            inputElement.dispatchEvent(new Event('input')); // Nếu cần cập nhật tooltip/UI
        } else {
            StatusUpdateBox("Web-Tools 3.0", "Không tìm thấy input với id: " + id, "red");
        }
    } catch (error) {
        StatusUpdateBox("Web-Tools 3.0", "Không thể đọc clipboard. Trình duyệt không cho phép hoặc chưa copy gì.", "red");
    }
...
// Hàm xử lý khi bấm vào nút "Combo Active" trong phần tìm kiếm khách hàng
document.getElementById('combo-active-button').addEventListener('click', function () {
    ...
    const execute = () => {
        ...
        if (!infoText || infoText === 'Không lọc được số') {
            StatusUpdateBox("Web-Tools 3.0", "Không có số ID tiệm", "red");
            return;
        }
...
// Hàm xử lý khi bấm vào nút "Save" trong phần chỉnh sửa thông tin người dùng
document.getElementById('save-user-infomation').addEventListener('click', function () {
    ...
    if (!newNickname || !newPassword) {
        ...
        StatusUpdateBox("Web-Tools 3.0", "Tên và Password phải có đủ", "red");
        return;
    }
    ...
    fetch('https://script.google.com/macros/s/AKfycbxy2wam-fhMZKGwB5LrJWTT-sCzpCytwZhUIcjBhmx9SojWSuAwVudAPCRx0CN7488/exec', {
        ...
        .then(result => {
            ...
            StatusUpdateBox("Web-Tools 3.0", "Cập nhật thông tin thành công!", "green");
    ...
    if (themeSwitchCheckbox) {
        themeSwitchCheckbox.addEventListener('change', function () {
            ...
            StatusUpdateBox("Web-Tools 3.0",
                enabled ? "Chế độ TỐI đã được <b style='color: orangered; font-size: 2rem;'>BẬT</b>" :
                    "Chế độ TỐI đã được <b style='color: orangered; font-size: 2rem;'>TẮT</b>",
                "green");
        });
    }
    updateDarkModeUI();
...
document.getElementById('Push-Refresh-All-User').addEventListener('click', function () {
    ...
    StatusUpdateBox("Web-Tools", "Đang gửi yêu cầu...", "orange");
    fetch(scriptURL, {
        method: 'POST',
        headers: {
        ...
        StatusUpdateBox("Web-Tools", "Yêu cầu đã được gửi thành công!", "green");
        console.log('Push refresh signal sent with timestamp:', data.timestamp);
    })
        .catch(error => {
            StatusUpdateBox("Web-Tools", "Đã có lỗi khi gửi yêu cầu.", "red");
...
function startChecking() {
  // Random từ 5s đến 30s (5000ms - 30000ms)
  const randomDelay = Math.floor(Math.random() * (30000 - 5000 + 1)) + 5000;

  setTimeout(async () => {
    try {
      await checkRefreshTimestamp();
    } catch (err) {
      console.error(err);
    }

    // Gọi lại chính nó để tiếp tục loop
    startChecking();
  }, randomDelay);
}
...
document.getElementById("refresh-onboard").addEventListener("click", function () {
    ...
    setTimeout(function () {
        ...
        StatusUpdateBox("Lịch Onboard", "Cập nhật thành công", "green");
    }, 2000);
});



document.getElementById('sua-ten-sheet').addEventListener('keydown', function (e) {
    if (e.key === 'Enter') {
        e.preventDefault(); // tránh submit form nếu có
        document.getElementById('updateA1Btn').click();
    }
});
...
document.getElementById('updateA1Btn').addEventListener('click', function () {
    ...
    if (!sheetName) {
        StatusUpdateBox("Lịch Trực Cuối Tuần", "Vui lòng nhập đầy đủ thông tin.", "red");
        return;
    }
    ...
    var scriptURL = 'https://script.google.com/macros/s/AKfycbwAsat4W2VRwVB1niazCDyNRTTd9EXagnC68BcwSYl7EhXZKqFBXSwiUG07ZKoXqh36iQ/exec';

    fetch(scriptURL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ sheetName }).toString()
    })
        .then(res => res.text())
        .then(() => {
            return new Promise((resolve, reject) => {
                setTimeout(() => {
                    fetch(
                        'https://script.google.com/macros/s/AKfycbxy2wam-fhMZKGwB5LrJWTT-sCzpCytwZhUIcjBhmx9SojWSuAwVudAPCRx0CN7488/exec?action=getLichTrucCopy',
                        { method: 'GET' }
                    )
                        .then(resolve)
                        .catch(reject);
        ...
        .then(data => {
            ...
            let table = '<table border="0" cellpadding="5" cellspacing="0">';
            ...
            for (let i = 0; i < data[0].length; i++) {
                ...
                <th>
                    <div style="display:flex;flex-direction:column;align-items:center;gap:1rem">
                        <div class="white-delete-icon" title="Xoá cột"></div>
                        <span>${data[0][i]}</span>
                    </div>
                </th>`;
                }
                ...
                for (let j = 0; j < data[i].length; j++) {
                    ...
                    <td style="padding:0">
                        <div style="display:flex;align-items:center;gap:.5rem">
                            <div class="white-delete-icon"></div>
                            <div class="orange-move-icon-2"></div>
                            <span style="flex:1">${data[i][j]}</span>
                        </div>
                    </td>`;
                    } else {
            ...
            const tbody = targetView.querySelector('tbody');

            tbody.querySelectorAll('td').forEach(td => {
                const text = td.textContent.trim().toLowerCase();
                if (text.includes("lịch trực")) {
                    td.closest('tr').classList.add('th-inside-tbody');
                }
                if (text.includes("software support")) {
            ...
            targetView.querySelectorAll('.th-inside-tbody').forEach(tr => {
                tr.querySelectorAll('.white-delete-icon, .orange-move-icon-2').forEach(el => el.remove());
            });

            (function removeTongColumn() {
                const tableEl = targetView.querySelector('table');
                if (!tableEl) return;
                ...
                ths.forEach((th, i) => {
                    ...
                    if (span && span.textContent.toLowerCase().includes("tổng")) {
                        index = i;
                    }
                });
            ...
            addMoveIconFunctions();

        })
        .catch(error => {
            console.error(error);
            StatusUpdateBox("Lịch Trực Cuối Tuần", "Đã có lỗi xảy ra.", "red");
            targetView.innerHTML = `<p style="color:red">${error.message}</p>`;
        })
        .finally(() => {
            loaderupdatelichtruc.classList.add('hidden');
            updateidlichtruc.classList.remove('hidden');
            StatusUpdateBox("Lịch Trực Cuối Tuần", "Đã đổi tháng trực thành công", "green");
        });
});
...
// Nút refresh Lịch Trực Cuối Tuần
    ...
    fetch('https://script.google.com/macros/s/AKfycbxy2wam-fhMZKGwB5LrJWTT-sCzpCytwZhUIcjBhmx9SojWSuAwVudAPCRx0CN7488/exec?action=getLichTrucCopy', {
        ...
        .then(data => {
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                for (let j = 0; j < data[i].length; j++) {
                    if (j === 0) {
                        table += '<td style="padding: 0; margin: 0;">' +
                            '<div style="display: flex; align-items: center; justify-content: space-between;">' +
                            '<div class="white-delete-icon" title="Xoá hàng"></div>' +
                            '<div class="orange-move-icon-2" title="Di chuyển hàng"></div>' +
                            '<span style="flex: 1;">' + data[i][j] + '</span>' +
                            '</div>' +
                            '</td>';
                    } else {
            ...
            const tbody = document.querySelector('#lich-truc-cuoi-tuan-view tbody');
            ...
            tbody.querySelectorAll('td').forEach(td => {
                const text = td.textContent.trim().toLowerCase();
                if (text.includes("software support")) {
                    const trToRemove = td.closest('tr');
                    if (trToRemove) {
                        trToRemove.remove();
                    }
                }
            ...
            document.querySelectorAll('#lich-truc-cuoi-tuan-view .th-inside-tbody').forEach(tr => {
                const firstTd = tr.querySelector('td:first-child');
                if (firstTd) {
                    const deleteIcon = firstTd.querySelector('.white-delete-icon');
                    if (deleteIcon) deleteIcon.remove();

                    const moveIcon = firstTd.querySelector('.orange-move-icon-2');
                    if (moveIcon) moveIcon.remove();
                }
            });

            (function () {
                const tableElement = document.querySelector('#lich-truc-cuoi-tuan-view table');
                if (!tableElement) return;

                const thList = tableElement.querySelectorAll('thead th');
                ...
                if (columnIndexToRemove === -1) return;

                thList[columnIndexToRemove].remove();
        ...
        .catch(error => {
            ...
            StatusUpdateBox("Lịch Trực Cuối Tuần", "Lỗi khi tải dữ liệu", "red");
            targetView.innerHTML = `<p style="color: red;">Lỗi khi tải dữ liệu: ${error.message}</p>`;
        })
        .finally(() => {
            loaderlichtruc.classList.add('hidden');
            StatusUpdateBox("Lịch Trực Cuối Tuần", "Đã cập nhật xong", "green");
        });
}



// Nút refresh Annoucements
document.getElementById("refresh-annoucement-container").addEventListener("click", async function () {
    ...
    StatusUpdateBox("Annoucements", "Đã cập nhật xong", "green");
});



// Nút refresh More Document NEW
...
// Mở popup yêu cầu quyền truy cập
...
// Lưu dữ liệu vào localStorage mỗi khi người dùng gõ phím
...
function loadLichTrucCopy() {
    fetch('https://script.google.com/macros/s/AKfycbxy2wam-fhMZKGwB5LrJWTT-sCzpCytwZhUIcjBhmx9SojWSuAwVudAPCRx0CN7488/exec?action=getLichTrucCopy', {
        ...
        .then(data => {
            ...
            var table = '<table border="0" cellpadding="5" cellspacing="0">';
...
function addMoveIconFunctions() {
    const tbody = document.querySelector('#lich-truc-cuoi-tuan-view tbody');
    let draggingRow = null;
    let isManualDragging = false;

    document.querySelectorAll('#lich-truc-cuoi-tuan-view tr').forEach(tr => {
        tr.draggable = false;
    });

    document.querySelectorAll('#lich-truc-cuoi-tuan-view .orange-move-icon-2').forEach((icon) => {
        const tr = icon.closest('tr');
    ...
    function handleMouseMove(e) {
        if (!isManualDragging || !draggingRow) return;

        const afterElement = getAfterRow(tbody, e.clientY);
    ...
    function handleMouseUp() {
        if (isManualDragging && draggingRow) {
...
async function fetchInitMainInterface() {
    const scriptURL = 'https://script.google.com/macros/s/AKfycbxy2wam-fhMZKGwB5LrJWTT-sCzpCytwZhUIcjBhmx9SojWSuAwVudAPCRx0CN7488/exec?action=initMainInterface';
...
function renderCheckIDInformation(result) {
    ...
    result.forEach((item, index) => {
        ...
        const DescriptionOfLink = item[2];
        ...
        const normalizedName = item[0].replace(/\s+/g, '-');
        ...
        group.appendChild(descriptionDiv);
...
//             const name = item[0];
//             const linkUrl = item[1];
//             const DescriptionOfLink = item[2]; // thêm mô tả
...
//             // Tạo name dựa trên item[0]
//             const normalizedName = item[0].replace(/\s+/g, '-'); // thay khoảng trắng bằng dấu -
//             input.name = 'Check-Id-Info-' + normalizedName;

//             // Lưu trạng thái checkbox theo name
//             input.checked = localStorage.getItem('checkboxState-' + normalizedName) === 'checked';
//             input.addEventListener('change', () => {
//                 localStorage.setItem('checkboxState-' + normalizedName, input.checked ? 'checked' : 'unchecked');
//             });
...
//                 // Delay 3 giây mới hiện descriptionDiv
//                 hoverTimeout = setTimeout(() => {
//                     descriptionDiv.style.left = mouseX + 'px';
//                     descriptionDiv.style.top = (mouseY - 10) + 'px';
//                     descriptionDiv.style.display = 'block';
...
//             // Khi rời chuột thì hủy hiện hoặc ẩn descriptionDiv
//             infoButton.addEventListener('mouseleave', () => {
//                 clearTimeout(hoverTimeout);
//                 descriptionDiv.style.display = 'none';
//             });
//             // Gom tất cả vào group
//             const group = document.createElement('div');
//             group.style.position = 'relative';
//             group.style.marginBottom = '1rem';
//             group.appendChild(link);
//             group.appendChild(label);
//             group.appendChild(infoButton); // thêm nút "?" vào dưới label
//             group.appendChild(descriptionDiv); // thêm mô tả (ẩn mặc định)
...
document.getElementById('save-button-for-BoxEditPersonalSMS').addEventListener('click', () => {
    ...
    StatusUpdateBox("My Personal SMS", "Đang gửi yêu cầu...", "orange");
    savePersonalSMS();
});
...
document.getElementById('DeleteBtn_Editing_MoreDoc_NewVer').addEventListener('click', async () => {
    ...
    try {
        ...
        if (result.includes("Row updated successfully")) {
            StatusUpdateBox("Add New Row", "Đã gửi yêu cầu <b style='color: orangered; font-size: 2rem;'>XOÁ</b>! Chờ duyệt từ Leader.", "green");
        } else {
...
document.querySelector('#SaveBtn_RequestAddRow_MoreDocTable_NewVer').addEventListener('click', async () => {
    ...
    if (isNotLoggedIn && isContentIncomplete) {
        ...
        StatusUpdateBox("Add New Row", "Thiếu Tiêu đề hoặc Nội dung.", "red");
        return;
    }
    ...
    try {
        ...
        if (result.toLowerCase().includes("row") && result.toLowerCase().includes("added")) {
            StatusUpdateBox("Add New Row", "Lưu thành công! Chờ duyệt từ Leader.", "green");
        } else {
            StatusUpdateBox("Add New Row", "Lỗi khi lưu: " + result, "red");
            return;
        }
...
document.getElementById('SaveBtn_Editing_MoreDoc_NewVer').addEventListener('click', async () => {
    ...
    try {
        ...
        if (result.toLowerCase().includes("updated")) {
            StatusUpdateBox("Add New Row", "Đã gửi yêu cầu <b style='color: orangered; font-size: 2rem;'>SỬA</b>! Chờ duyệt từ Leader.", "green");
        } else {
            StatusUpdateBox("Add New Row", "Lỗi khi lưu: " + result, "red");
            return;
        }
...
function StaffFilterButton() {
    const btn = document.getElementById('Staff-Filter-Button');
    const box = document.getElementById('Staff-Filter-Container');
...
function ChamCongLichTruc(data) {
    const target = document.getElementById('cham-cong-lich-truc');
    if (!target || !Array.isArray(data) || data.length < 2) return;

    const header = data[0];

    const DEPARTMENT = 'Software Support';
    const TITLE = 'Nhân viên hỗ trợ kỹ thuật';

    // 👉 Set lưu tên duy nhất
    ...
    let html = `
        <table border="0" cellpadding="5" cellspacing="0">
            <thead>
                <tr>
                    <th>Tên (lấy từ Lịch Trực,<br>nên chỉnh sửa lại khi copy)</th>
                    <th>Phòng</th>
                    <th>Chức vụ</th>
                    <th>Ngày làm việc</th>
                    <th>Ngoài giờ từ</th>
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        const name = row[0];
        if (name && name.includes('LỊCH TRỰC THÁNG')) continue;

        // 👉 Chỉ giữ tên duy nhất
        if (name && name.trim() !== '') {
        ...
        for (let col = 1; col < row.length; col++) {
            const shift = row[col];
            const headerText = header[col];

            const shiftStr = (shift ?? '').toString().trim();
            ...
            const timeMatch = shiftStr.match(/(\d{1,2}:\d{2}\s?[AP]M)\s*-\s*(\d{1,2}:\d{2}\s?[AP]M)/i);
    ...
    TakeNameToFilterStaffContainer([...uniqueNames].sort());
}


function TakeNameToFilterStaffContainer(nameList) {
    const container = document.getElementById('Staff-Name-In-Filter-Container');
    if (!container || !Array.isArray(nameList)) return;

    let html = '';

    // All Staffs
    html += `
        <label class="checkbox-container" style="height: auto;">
            <input 
                type="checkbox"
                class="custom-checkbox"
                name="Check-Staff-Name"
                value="All Staffs"
                data-staff-name="All Staffs"
                checked
            >
            <div class="checkmark"></div>
            <span class="staff-name-text">All Staffs</span>
        </label>
    `;
    ...
    nameList.forEach(name => {
        html += `
            <label class="checkbox-container" style="height: auto;">
                ...
                >
                <div class="checkmark"></div>
                <span class="staff-name-text">${name}</span>
            </label>
        `;
    });
    ...
    const checkboxes = container.querySelectorAll('input[name="Check-Staff-Name"]');
    const allStaffCheckbox = container.querySelector('input[value="All Staffs"]');

    function filterTable() {
        const table = document.querySelector('#cham-cong-lich-truc table');
        if (!table) return;

        const rows = table.querySelectorAll('tbody tr');

        // Danh sách staff đang được bật
        const enabledStaff = [...checkboxes]
            .filter(cb => cb.checked && cb.value !== 'All Staffs')
            .map(cb => cb.value);
        ...
        rows.forEach(tr => {
            ...
            tr.style.display = enabledStaff.includes(staffName)
                ? ''
                : 'none';
        });
    }
    ...
    checkboxes.forEach(cb => {
        cb.addEventListener('change', () => {
            ...
            // Click All Staff
            if (cb.value === 'All Staffs') {
                checkboxes.forEach(item => {
                    item.checked = cb.checked;
                });
                const ghiNhanText = cb.checked
                    ? "Bấm vào <b style='color:orange'>All Staffs</b>,<br>ghi nhận <b style='color:orange'><u>CÓ</u></b> Chọn"
                    : "Bấm vào <b style='color:orange'>All Staffs</b>,<br>ghi nhận <b style='color:orange'><u>KHÔNG</u></b> Chọn";

                StatusUpdateBox("Chấm Công Lịch Trực", ghiNhanText, "green");
                filterTable();
                return;
            }

            // Click staff riêng lẻ
            const staffCheckboxes = [...checkboxes].filter(c => c.value !== 'All Staffs');
            const allChecked = staffCheckboxes.every(c => c.checked);
            allStaffCheckbox.checked = allChecked;
            const staffName = cb.value;
            const ghiNhanTextStaffLe = cb.checked
                ? `Bấm vào <b style='color:orange'>${staffName}</b>,<br>ghi nhận <b style='color:orange'><u>CÓ</u></b> Chọn`
                : `Bấm vào <b style='color:orange'>${staffName}</b>,<br>ghi nhận <b style='color:orange'><u>KHÔNG</u></b> Chọn`;


            StatusUpdateBox("Chấm Công Lịch Trực", ghiNhanTextStaffLe, "green");
            filterTable();
        });
    });

    // ===== SEARCH STAFF NAME =====
    const searchInput = document.getElementById('find-staff-name-in-filter');
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            const keyword = this.value.trim().toLowerCase();

            const labels = container.querySelectorAll('.checkbox-container');

            labels.forEach(label => {
                const nameSpan = label.querySelector('.staff-name-text');
                if (!nameSpan) return;

                const nameText = nameSpan.textContent.toLowerCase();

                // Luôn hiện All Staff
                if (nameText === 'all staffs') {
                    label.style.display = '';
                    return;
                }

                label.style.display = nameText.includes(keyword)
                    ? ''
                    : 'none';
            });
        });
...
function CopyAllShownRows() {
    console.log('🟢 CopyAllShownRows called');

    const table = document.querySelector('#cham-cong-lich-truc table');
    if (!table) {
        console.warn('❌ Không tìm thấy table');
        return;
    }
    ...
    rows.forEach(tr => {
        ...
        const rowText = [...cells]
            ...
            .join('\t'); // tab cho Excel / Sheets

        output += rowText + '\n';
    });

    if (!output.trim()) {
        alert('Không có dòng nào đang hiển thị để copy.');
        return;
    }

    navigator.clipboard.writeText(output)
        .then(() => {
            console.log('✅ Copied visible rows');
            StatusUpdateBox("Lịch Trực - Chấm Công Lịch Trực", "Đã copy toàn bộ dòng", "green");
        })
        .catch(err => {
...
function initChamCongToggle() {
    const checkbox = document.querySelector(
        'input[name="Checkbox-Id-dien-cham-cong-lich-truc"]'
    );

    const lichTruc = document.getElementById("lich-truc-cuoi-tuan-container");
    const chamCong = document.getElementById("cham-cong-Big-container");

    if (!checkbox || !lichTruc || !chamCong) return;

    const applyState = () => {
        if (checkbox.checked) {
            lichTruc.style.display = "none";
            chamCong.style.display = "";
            StatusUpdateBox("Lịch Trực Cuối Tuần", "Xem <b style='color:orange'>Điền Chấm Công Lịch Trực</b>", "green");
        } else {
            lichTruc.style.display = "";
            chamCong.style.display = "none";
            StatusUpdateBox("Lịch Trực Cuối Tuần", "Về lại <b style='color:orange'>Lịch Trực Cuối Tuần</b>", "green");
        }
    };
    applyState();
