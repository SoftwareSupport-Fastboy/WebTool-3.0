...
const MyNote = document.getElementById('My_Note');
const MyNotecontainer = document.querySelector('.My_Note_container');

clockContainer.addEventListener('click', function(event) {
    event.stopPropagation();
    converterTimeContainer.style.display = 'flex';
    MyNotecontainer.style.display = 'none';
});

MyNote.addEventListener('click', function(event) {
    event.stopPropagation();
    converterTimeContainer.style.display = 'none';
    MyNotecontainer.style.display = 'inline';
});

document.addEventListener('click', function(event) {
    // Kiểm tra nếu người dùng nhấn vào white-delete-icon
    if (
        !clockContainer.contains(event.target) &&
        !converterTimeContainer.contains(event.target) &&
        !MyNote.contains(event.target) &&
        !MyNotecontainer.contains(event.target) &&
