...
document.getElementById("findButton").addEventListener("click", function (event) {
    ...
    const baseUrl = "https://crm.fastboy.dev/clients/company?page=1&itemsPerPage=20";
    const urlParams = `postalCode=${zipCode}`;
    
    // Tạo các URL khác nhau tùy thuộc vào phone
    let finalUrl = baseUrl + "&" + urlParams;

    if (phone) { // Nếu Phone có giá trị
        const urlParams2 = `postalCode=${zipCode}&contactCompanies.contact.phone=${phone}`;
        const urlParams3 = `postalCode=${zipCode}&businessPhone=${phone}`;
        finalUrl = baseUrl + "&" + urlParams2;
        // Mở cả 3 liên kết nếu có phone
        window.open(finalUrl, "_blank");
        window.open(baseUrl + "&" + urlParams3, "_blank");
    } else { // Nếu không có phone
        window.open(finalUrl, "_blank");
    }
});
...
function fetchSMSData() {
    const url = 'https://script.google.com/macros/s/AKfycbxy2wam-fhMZKGwB5LrJWTT-sCzpCytwZhUIcjBhmx9SojWSuAwVudAPCRx0CN7488/exec'; // Địa chỉ URL tới backend của bạn
    const params = {
        action: 'getSMSData'
    };
...
document.getElementById('combo-active-button').addEventListener('click', function() {
    ...
    checkboxes.forEach((checkbox, index) => {
        const link = checkbox.previousElementSibling; // lấy <a> tương ứng với checkbox
        const linkUrl = link.href;

        if (checkbox.checked) {
            // Chạy pasteClipboard để lấy giá trị từ clipboard và cập nhật vào input
            pasteClipboard().then(() => {
                // Chạy updateCheckInfo với giá trị mới từ input
                const customValues = document.getElementById('id-of-check-id-information')?.textContent.split(',');
                ...
                if (customValues && customValues.length > 0) {
                    customValues.forEach(customValue => {
                        ...
                        // Mở liên kết đã chỉnh sửa trong tab mới
                        window.open(modifiedLinkUrl, '_blank');
                    });
                } else {
                    console.log("No custom values found in span.");
